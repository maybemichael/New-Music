...
class DismissAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        UIView.animate(withDuration: transitionDuration(using: transitionContext), delay: 0, usingSpringWithDamping: 1.0, initialSpringVelocity: -5, options: .curveLinear) { [weak self] in
...
//        UIView.animate(withDuration: transitionDuration(using: transitionContext), delay: 0, options: .curveEaseIn) { [weak self] in
//            guard let self = self else { return }
//            self.musicController.nowPlayingViewModel.isFull = false
//            fromVC.view.frame = self.finalFrame
//            fromVC.view.layer.cornerRadius = 7
